  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Title */}
        <p style={{ margin: 0, fontSize: '19px', fontWeight: 700, lineHeight: 1 }}>
          TOP 3% TALENT
        </p>

        {/* Divider */}
        <div style={{ width: '120px', height: '1px', background: '#25a9ef' }} />

        {/* Vetted by */}
        <span style={{ fontSize: '16px', marginBottom: '-6px' }}>Vetted by</span>
  ...
  return (
    <div className="p-6 md:p-10">
      ...
      <motion.div
        ...
        className="mb-10"
      >
        <h1 className="text-white text-3xl md:text-4xl font-bold">Certifications</h1>
        <h2
      ...
      {/* Toptal feature block */}
      <motion.div
        className="border border-zinc-700/40 rounded-xl p-8 md:p-12 mb-10 bg-zinc-900/50 flex flex-col md:flex-row items-center gap-10"
      ...
      >
        ...
        <div className="flex-1">
          ...
          <h3 className="text-white text-2xl md:text-3xl font-bold mb-3">Toptal Vetted Expert</h3>
          <p className="text-zinc-300 leading-relaxed mb-6">
            Toptal accepts only the top 3% of applicants after a rigorous screening process covering
            language & personality tests, in-depth technical interviews, live problem-solving, and
            test projects. Being part of this network means every client gets senior-level quality
            from day one.
          </p>
          <a
            href="https://www.toptal.com/developers/resume/haris-zahid#M59VDn"
            target="_blank"
            rel="noopener noreferrer"
            className="items-center gap-2 font-semibold text-sm px-5 py-3 rounded-lg transition-opacity hover:opacity-80 flex"
      ...
      {certifications.length > 0 && (
        ...
        >
          <h3 className="text-white text-xl font-bold mb-6 border-l-2 border-amber-400 pl-3">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {certifications.map((cert, i) => (
              <motion.div
                key={cert.title}
                className="border border-zinc-700/40 rounded-lg p-5 bg-zinc-900/50 hover:border-indigo-500/40 transition-colors"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.5, delay: 0.4 + i * 0.08 }}
              >
                <p className="text-white font-semibold mb-1">{cert.title}</p>
                <p className="text-zinc-400 text-sm mb-1">{cert.issuer}</p>
                {cert.date && <p className="text-zinc-500 text-xs mb-3">{cert.date}</p>}
                {cert.url && (
                  <a
                    href={cert.url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-1 text-indigo-400 text-xs hover:text-indigo-300 transition-colors"
                  >
